...
The integration of autonomous robots into our daily environments is increasing significantly for reducing cost based on logistics of packages\cite{5262912}\cite{PEREZGRAU2021312} and use in different environments like hospitals\cite{technologies9010008}\cite{10.1007/978-3-030-57993-7_76}, mining \cite{grehl2015mining}, warehouse\cite{poudel2013coordinating}\cite{keith2024reviewautonomousmobilerobots} and agricultural\cite{machines11010048}. Expectialy in office builfing with iteration with human is a costant the lack of the automated guided vehicles (AGV) cannot bypass obstacles and enter wards or departments due to safety and space concerns so this require the use of AMRs\cite{10797381}.
There are multiple type of AMR and different type of locomotion: wheeled robots, tracked robots and legged robots. Since the wheeled robot have the following advantages , they are energy efficiency, can have an higher payload\cite{tagliavini2022wheeled}, are more stable\cite{doi:10.1177/1729881419839596}, have a lower mechanical complexity and are easy to control in a structured environment\cite{ms-3-49-2012} so they are the oprimal solution for the task at the office.
The wheeled robot can have different model of locomotion:differente drive (that use the differential steering or skid steering twcnique), omni wheel, and Ackermann.
Given the advantages given by the differeent drive like more payload the different drive structure is used for robotics application. \cite{5427035}\cite{tagliavini2022wheeled}
...
Among the projects built on ROS, Nav2\cite{macenski2020marathon2} stands out for its ability to manage autonomous navigation\cite{example1nav2}\cite{8735597}. Given a position estimation of the robot in a map that can be obtained through a state estimation using sensor data \cite{MooreStouchKeneralizedEkf2014} combinated with a Simultaneous Localization And Mapping tools (SLAM)\cite{45466}\cite{Macenski2021}\cite{vslamComparison2021}\cite{sh-p1-prelude} or just using localization\cite{AMCL_base_teoria}\cite{das2025robotlocalizationmappedenvironment} allow a robot to navigate in the environment(see \ref{lab:nav2}).
In our current work, we aim to control two of our Unmanned Ground Vehicle (UGV) using Nav2. Its modularity allows for the evaluation of multiple global \cite{macenski2023survey}\cite{10.1007/978-3-031-67295-8_33}  and local \cite{580977}\cite{7487277} planners.\\
Since we have multiple robots to control a solution for managing heterogeneous fleet of robots is needed. Additionally, AGVs effectively managed choke points with
fleet management software to prevent congestion, whereas
AMRs struggled without coordination, causing delay\cite{10797381}.  Among the available options (see \ref{lab:rmf}), we will use the Open Robotics Middleware Framework (Open-RMF)(see \ref{lab:rmf}), a middleware for robots that has obtained substantial interest\cite{salzillo2024robot}\cite{10773345} for its capabilities in traffic management and parallel task execution.
\newpage

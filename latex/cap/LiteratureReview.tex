...
Given the necessity to manage multiple robots, a solution for overseeing a heterogeneous fleet of robots is required. Additionally, while AGVs effectively manage choke points with fleet management software to prevent congestion, AMRs often struggle without such coordination, resulting in delays\cite{10797381}.
Among the available options (see \ref{lab:rmf}), we will use the Open Robotics Middleware Framework (Open-RMF)(see \ref{lab:rmf}), a middleware for robots that has obtained substantial interest\cite{salzillo2024robot}\cite{10773345} for its capabilities in traffic management and parallel task execution.
\newpage
...
\begin{itemize}
	...
	\item \textbf{Multiple Client Libraries}: Allow to program in ROS using multiple languages and access to the core communication APIs.
	\begin{figure}[H]
		\centering
...
\subsection{Open-RMF description}
...
\begin{itemize}
	...
	\item \texttt{rmf\_obstacle}: Multiple packages that allow an interaction between the presence of obstacles or humans via sensors and the lanes of the fleet navigation graph.
	\newpage

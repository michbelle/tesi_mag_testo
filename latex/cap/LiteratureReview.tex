...
\begin{itemize}
	...
	\item \alert{\textbf{Multiple Client Libraries}: Allow to program in ROS using multiple languages and access to the core communication APIs}.
	\begin{figure}[H]
		\centering
...
\begin{itemize}
	...
	\end{figure}
	\alert{Nodes can also be instantiate into process using the \textbf{Composition}\label{Composition} feature, which also allow bypassing middleware overhead through intra-process communication, reducing RAM and CPU usage while improving goodput and latency}\cite{macenski2023impactros2node}.
...
\begin{itemize}
	...
	\item \textbf{nav2\_util LifecycleNode}: This is a wrapper around the standard\\ \texttt{rclcpp\_lifecycle::LifecycleNode} described earlier \ref{LifecycleNode}. It adds a bond connection to the Lifecycle Manager \alert{node}, which monitors the state of each node and can safely transition nodes through their lifecycle states in the event of a failure.
...
\end{itemize}
\alert{If a fleet does not belong to one of these categories, it falls under the classification of \textit{No Interface} and is incompatible with Open-RMF, as this could lead to deadlocks in resource allocation.}
\\
...
\begin{itemize}
	...
	\item \remove{\texttt{rmf\_traffic\_ros2}: Provides the necessary ROS 2 interfaces for integrating rmf\_traffic into ROS 2-based systems.}
	...
	\item \alert{\texttt{rmf\_obstacle}: Multiple packages that allow an interaction between the presence of obstacles or humans via sensors and the lanes of the fleet navigation graph. }
...
\begin{itemize}
	...
	\end{figure}
	\alert{From this package was developed \texttt{free\_fleet} for controlling robots developed in ROS 1 and ROS 2.}
	...
	\item \alert{\texttt{door\_adapter\_template} and \texttt{lift\_adapter\_template}: Templates for writing a python based Open-RMF door and lift adapter.}

...
The integration of autonomous robots into our daily environments is increasing significantly. These robots leverage various proprietary software solutions alongside open-source platforms such as Ardupilot and Robot Operating System ROS\cite{quigley2009ros} to navigate in the environment. Originally developed as an academic project, ROS has greatly expanded its application scope. Transitioning to ROS 2\cite{Macenski_2022} introduced new features for data transfer\cite{pardo2005introduction}, executing nodes\cite{macenski2023impactros2node},lifecycle node management  and security\cite{vilches2022sros2usablecybersecurity}, which facilitate its adoption in production environments (see \ref{lab:ros} and \ref{lab:port12}).\\
...
Since we have multiple robots to control a solution for managing heterogeneous fleet of robots is needed.  Among the available options (see \ref{lab:rmf}), we will use the Open Robotics Middleware Framework (Open-RMF)(see \ref{lab:rmf}), a middleware for robots that has obtained substantial interest\cite{salzillo2024robot}\cite{10773345} for its capabilities in traffic management and parallel task execution.
\newpage

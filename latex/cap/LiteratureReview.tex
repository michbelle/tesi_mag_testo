...
The integration of autonomous robots into our daily environments is increasing significantly to reduce costs associated with package logistics\cite{5262912}\cite{PEREZGRAU2021312} and their use in various environments such as hospitals\cite{technologies9010008}\cite{10.1007/978-3-030-57993-7_76}, mining \cite{grehl2015mining}, warehouses\cite{poudel2013coordinating}\cite{keith2024reviewautonomousmobilerobots} and agriculture\cite{machines11010048}. For a long period, the solution for moving objects in various industries has been the use of automated guided vehicles (AGVs). These vehicles rely on markers or laser systems to navigate their surroundings. However, in environments such as office buildings, where interaction with humans is constant, the use of automated guided vehicles (AGVs) poses challenges. AGVs often cannot bypass obstacles or cannot enter some areas due to safety and space concerns, so it necessitates the use of autonomous mobile robots (AMRs)\cite{10797381} with their more autonomy capabilities can overcome this challenges.
There are several type of AMRs that can be divided by the type of locomotion: wheeled robots, tracked robots and legged robots. Since wheeled robots offer several advantages, such as energy efficiency, higher payload capacity\cite{tagliavini2022wheeled}, increased stability \cite{doi:10.1177/1729881419839596}, lower mechanical complexity, and ease of control in structured environments\cite{ms-3-49-2012} they are the optimal solution for tasks that are performed in a office.
Wheeled robots can adopt various locomotion models: different drive systems (which utilize differential steering or skid steering techniques), omni-wheels and Ackermann steering. 
Given the advantages of the different drive, like more payload and more maneuverability without the need of a mechanical steering subsystem, the different drive structure is the most used for robotics application \cite{5427035}\cite{tagliavini2022wheeled} in small spaces.\\
...
In our current work, we aim to control two different drive Unmanned Ground Vehicle (UGV) using Nav2. Its modularity allows for the evaluation of multiple global \cite{macenski2023survey}\cite{10.1007/978-3-031-67295-8_33} and local \cite{580977}\cite{7487277} planners.\\
...
\begin{itemize}
	...
	\item \textbf{Multiple Client Libraries}: Allow to program with ROS using multiple languages and access to the core communication APIs.
	\begin{figure}[H]
		\centering
...
\begin{itemize}
	\item \textbf{Nodes}: A node is a modular process that should performs a specific task. Nodes communicate with each other through topics, services, actions and parameters. In ROS 2, nodes can be implemented as \textbf{LifecycleNode}\label{LifecycleNode}, which provide a managed state machine with defined transitions for startup and shutdown, enabling more deterministic behavior.
	\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{img/life_cycle_sm_nf.png}
	\captionof{figure}{State Machine of LifecycleNode}
	\end{figure}
	Nodes can also be instantiate into process using the \textbf{Composition}\label{Composition} feature, which also allow bypassing middleware overhead through intra-process communication, reducing RAM and CPU usage while improving goodput and latency\cite{macenski2023impactros2node}.
...
\begin{itemize}
	...
	\item \textbf{nav2\_util LifecycleNode}: This is a wrapper around the standard\\ \texttt{rclcpp\_lifecycle::LifecycleNode} described earlier \ref{LifecycleNode}. It adds a bond connection to the Lifecycle Manager node, which monitors the state of each node and can safely transition nodes through their lifecycle states in the event of a failure.
...
\begin{itemize}
	...
	\item \textbf{map} can either be preloaded or built dynamically using a SLAM method. One SLAM method systems is Google Cartographer, which will be described in the next section. It is also the SLAM method that was used to generate the map we use for navigation.
\end{itemize}
\newpage
...
\end{itemize}
If a fleet does not belong to one of these categories, it falls under the classification of \textit{No Interface} and is incompatible with Open-RMF, as this could lead to deadlocks in resource allocation.
\\
...
\begin{itemize}
	...
	\item \texttt{rmf\_obstacle}: Multiple packages that allow an interaction between the presence of obstacles or humans see via external sensors and the lanes of the fleet navigation graph.
	\newpage
	\item \texttt{rmf\_ros2}: Supplies ROS 2 adapters, nodes, and Python bindings to integrate \texttt{rmf\_core} components into ROS 2 distribute system, as show in the following picture.

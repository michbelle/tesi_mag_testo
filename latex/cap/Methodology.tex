...
The steps taken to port the code in ROS 2 are described in the following section. When porting the system we used the same design for navigation and control that was used when build the first time\cite{10.1007/978-3-031-67295-8_33}. 
...
For launching the sensor and the robot similar code is used. In the package:
\begin{itemize}
	\item \texttt{jobot\_driver\_ros2}: there is the code for controlling the Jobot.
	\item \texttt{jobot\_launchpad}: the code used to launch Nav2.
	\item \texttt{jobot\_msgs}: the custom msgs used by Jobot. 
	\item \texttt{jobot\_simulation}: all the code to simulate the Jobot in Gazebo.
\end{itemize} 
...
\newpage
For the Mini, we followed a long path with many turns for changing the parameters associated with the \texttt{robot\_localization} package:\\
...
\end{figure}
\newpage
While for the Jobot, the path is the following one, it has similarities with the paths recorded for the Mini. After moving through the hallway, the doors were closed to evaluate how much will it affect the AMCL.\\
...
Other records were made for validating the parameter choises.\\
To record the data, we use the command \texttt{ros2 bag record}. While for playing, we need to publish to the \texttt{/clock} topic to avoid errors related to the difference in time between the recorded data and the system time: “the timestamp on the message is earlier than all the data in the transform cache.” This is facilitated by rosbag2, using the flag \texttt{--clock} that publishes to \texttt{/clock}. This requires the flag \texttt{use\_sim\_time} for all the nodes, just like in simulation. 
...
The picture show how big the error in rotation is for the Mini. For compensate this error we see what data our sensor are able to provide:
...
\begin{itemize}
	\item pose : it's not correct and add a lot of error in the estimation.
...
Changing $\alpha 1$ and $\alpha 2$ fixes the rotation error, but we observe significant oscillation in the position correction if we increase them too much; this is evident in data 004. In the end, we decided to keep both parameters around 0.01. As a result, we obtained this picture of the odometry, which shows that the rotation was corrected.
...
Because we didn't increase the correction for rotation during translation $\alpha 2$, and since we moved straight after the jump, AMCL did not fix the rotation error. So, we increased it, achieving good results. To obtain even better outcomes, we added a bit more correction with  $\alpha 3$ and $\alpha 4$. Considering that our test path was short and there could be additional errors in translation, we also observed an increase in estimation performance. The plot of the data for the best results is as follows:
...
At the vertical red line, it visible that the small step generated a very high error in all three dimensions: $x$, $y$ and $yaw$, but AMCL managed to correct it perfectly.
...
Now we tested the robots in simulation and discovered that for the Jobot, during sharp curves, as shown in figure \ref{sharp curves} below, the robot stopped right before the turn, rotated in place, and then proceeded to hit the wall, eventually stopping.
...
\alert{Also for control the robot is added to the systemd services for controlling the sequence of launch file to execute.}

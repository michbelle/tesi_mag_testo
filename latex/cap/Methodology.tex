...
During the testing, we saw that the performance of the odometry and the localization were need to be imporved with some parameters tuning.
in the following paragraphers it will describe the method used and the parameters behaviour comparisation. In the first we describe how the test were performed, then the behavior of the prameters on the robot\_localization node for improve the odometry estimation and the correction that the amcl node in function of its parameters.


\section{Testing environment using Rosbag}
For allow a fast approach on setting the parameters, was chose to record the data given from the robots and from their sensors using the tool Rosbag2
...
We choose some path that the robots should have followed and using the /cmd\_vel topic we moved them along the building T of Elettra. We followed this paths for the mini:\\

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\linewidth]{img/record_001_path_mini.png}
	\captionof{figure}{path followed during record 001 for the mini}
\end{figure}

while for the jobot the path is the following one:\\
...
other recorder were made for validating the parameter selection. To record the data we use the command \texttt{ros2 bag record}.
For playing that data, we need to make publish to the \texttt{/clock} topic to avoid errors related to the difference in time of the recorded data and the system time : \texttt{the timestamp on the message is earlier than all the data in the transform cache}. This is available by the rosbag2 using the flag \texttt{--clock} that publish to \texttt{/clock} at a specific frequency. 

Also for reducing the time to test we play the data with a rate 5 time with the following flag \texttt{-r 5} , this flag increase the output rate of each topic by 5 times. Test were performed with a rate of 1 and 5 for multiple record and the output show that the results of the robot\_localization and for the amcl were a lot similar in the two cases.\\

Now that we set the testing environment, the process of setting the parameters for the \texttt{robot\_localization} node.


\section{Parameters setting for the \texttt{robot\_localization} node}

For the analysis of the data of the \texttt{robot\_localization} we wanted that to visualize them in a clear way and without that amcl node influencing the visualization of the result of the \texttt{robot\_localization} node, so we set all the $\alpha$ parameters to 0.0 of the amcl node. This allow us to visualize the position of the walls during the movements of the robot allowing to better understand his position and the type of error that the odometry generates. when done the first correction and fixed the AMCL, then it possible to visualize the data of correction and see if our deduction where correct.

As described before, the \texttt{robot\_localization} contains an implementation of the extended Kalman filter that support multiple sensors and allow the user to select with data fields are fused with the current state estimation. Also since the process noise covariance matrix $Q$ can be difficult to tune, so it's given as parameters.\\
The theory behind is that we want to know the state $x$, given a non liner system $f$ with process noise $w$:
\begin{align}
	x_k &= f(x_{k-1})+w_{k-1}
\end{align}
...
In the next section we will descibe our procedure and how we tune them for the mini and for the jobot.
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|c|c|c|}
		...
		0 & 0 & 0 & 0 & 1.92 &  - \\
		\hline
		0.001 & 0 & 0 & 0 & 1.32 & 000 \\
		\hline
		0.01 & 0 & 0 & 0 & 0.60 & 001 \\
		\hline
		0.1 & 0 & 0 & 0 & 0.64 & 002 \\
		\hline
		0 & 0.001 & 0 & 0 & 0.58 & 003 \\
		\hline
		0 & 0.1 & 0 & 0 & 0.51 & 004 \\
		\hline
		0 & 0 & 0.001 & 0 & 2.01 & 005 \\
		\hline
		0 & 0 & 0.01 & 0 & 2.27 & 006 \\
		\hline
		0 & 0 & 0.1 & 0 & 2.32 & 007 \\
...
this is the data obtained with the 

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{img/record1.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{img/posizione giusta.png}

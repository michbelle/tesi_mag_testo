...
We, also, discovered that in both of the records, the robot while moving through a fire door make a "jump" visible in the odometry, this can be caused by a little step needed for the fire door. 
...
During testing (data 009) was possible to see that the compbination of these parameters allowed a correction on the position on the left of the path (the one with more map detail) not completely solved the problems on the right (with less detail) but was manage to correct it.
...
\subsection{setting the amcl of the jobot}

As visible in picture \ref{Ottimal result obtained visualized in Rviz}, the error to correct is angular position and this is create during rotation around of axis, so as described for the parameters we need to change the $\alpha 1$ parameter, as descibed before is \textit{Expected process noise in odometry’s rotation estimate from rotation}.

In both the record when the robot passed the door, it "jumped" and create an error in the estimation. We keep that information and see how the amcl package recover from that jump.
Also the start position and the end one has a difference of aounrd 0.6 m

This time we start with all parameter set to low and increasing the $\alpha 1$ parameter as follow and we describe the results:
\begin{table}[H]
	\centering
	\begin{tabular}{|c|c|c|c|c|c|}
		\hline
...
The data 000 see that the robot didint recover from that jump and also managed a little.

the incrementation of the paramenter in data 001 not reported un impovment, the jump break highlty the system.
...
we can try to solve this problem with adding fix rotation during translation incrementing parameter $\alpha 2$, record 002. 

we tried to increment $\alpha 3$ to have a better incremention of position error during translation, but added more error to the result.

so the optimal solution is the with:

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{img/record2.png}
\captionof{figure}{record2}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{img/amcl_jobot_best_plot.png}
\captionof{figure}{posizione giusta}
\end{figure}


\section{Navigation errors}

During the navigation is possible to see when performing strette turns , that the jobot was gettitng stuck.
...
This was solved, also reading on papers this is the best solution, changing the Local planner from the Dynamic Window Approach DWB successore del DWA (The goal of a local planner is to take the global plan and local costmap and produce the command velocities that will presumably move the robot to the goal. Both dwa and dwb do this via sampling, i.e. generating plausible velocity commands and evaluating them on various metrics and selecting the one with the best score, until the robot reaches its goal) to the Model Predictive Path Integral Controller MPPI \cite{7487277}, without any tuning of the controller the jobot performed better\cite{macenski2023survey}.

Also if not enought feature where found
the record also will not describe the reality, if we ask the planner to move to a location and the AMCL doest manage to correct in time the position the global planner becouse the position of the door in the global planner is not aligned with the position of the door in the local planner and the path is "blocked" for the navigation.
The solution is to add a openrmf vertex before the door to allow more iteration during rotation to fix the translation error allowing to generate a path to enter at the door.
Other solution is to apply some marker visible by a camera in the office space so to the localization node can have a periodic absolute correction, like a GPS (also that can be added to allow correction and movment outside the facility, moving between the buildings).


this is the type of error you can inconter:
...
\section{External enviromets detail}
...
A cheap solution is to add some ultrasonic sensor with an hight field of view to catch these object that the lidar doesnt see.

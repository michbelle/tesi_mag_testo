...
\begin{itemize}
	\item \texttt{0s\_ign\_spawn\_world\_gazebo.launch.py}: That launch gazebo with the a world.
	\item \texttt{1s\_ign\_spawn\_mini\_gazebo.launch.py}: Spawn the model of the robot in gazebo and with all sensors and launch the \texttt{ros\_gz\_bridge/parameter\_bridge} node is used to share simulated data between Gazebo and the ROS 2 environment.
\end{itemize}
\begin{figure}[h]
	\centering
...
The map create using Cartographer was obtained by the robot Jobot while navigating through Building T of Elettra, which serves as an office building. The map is as follows:
\begin{figure}[H]
	\centering
...
\begin{itemize}
	...
	\begin{itemize}
		\item As \texttt{controller\_plugins} use \texttt{nav2\_mppi\_controller::MPPIController}: a Model Predictive Path Integral (MPPI)\cite{7487277} algorithm to track a path with adaptive collision avoidance.
...
As previously mentioned, the \texttt{DStarGlobalPlanner}\footnote{\href{https://github.com/ElettraSciComp/DStar-Trajectory-Planner/tree/ros2_port}{https://github.com/ElettraSciComp/DStar-Trajectory-Planner/tree/ros2\_port}} is a implementation of the D* informed incremental search algorithm\cite{overview_Route_Planning_Algorithms} with external optimizers\cite{10.1007/978-3-031-67295-8_33} for the ROS 1 \texttt{move\_base} package\footnote{\href{https://wiki.ros.org/move_base}{https://wiki.ros.org/move\_base}} and we want to analyze if there are some advantages of using this planner compared to the default planner.\\
...
Additionally, we controlled the robots by publishing on the \texttt{/cmd\_vel}topic, following some paths that can be interesting to analyze.\\
\newpage
For the Mini, we followed this path for changing the parameters associated with the \texttt{robot\_localization} package; it’s a long path, and there are many turns:\\
...
and this for setting the parameters for the AMCL node because,on the right side of the door, along the corridor, the doors are closed that can reduce the robot's ability to accurately determine its position, making it crucial to fine-tune the AMCL parameters accordingly:
...
\end{figure}
\newpage
While for the Jobot, the path is the following one, it has similarities with the paths recorded for the Mini. After moving through the hallway, the doors were closed to evaluate the effect on the AMCL.\\
...
To record the data, we use the command \texttt{ros2 bag record}. While playing that data, we need to publish to the \texttt{/clock} topic to avoid errors related to the difference in time between the recorded data and the system time: “the timestamp on the message is earlier than all the data in the transform cache.” This is facilitated by rosbag2, using the flag \texttt{--clock} that publishes to \texttt{/clock}. This requires the flag \texttt{use\_sim\_time} for all the nodes, just like in simulation. 

To reduce the testing time, we played the data at a rate five times higher using the flag \texttt{-r 5} for \texttt{robot\_localization} , while for the AMCL we used \texttt{-r 2}; this flag increases the output rate of each topic by n times. Tests were performed with rates of 1 and 5 for multiple recordings, and the output showed that the results of \texttt{robot\_localization} were similar to those in the case of the AMCL tests.\\
...
The data available for the Mini are the mounted IMU and its estimated odometry.
...
Next are listed some information about these data from the:\\
...
We can see that the best results were achieved by incrementing the value of the process covariance matrix for  $v_{yaw}$, while changing the other inputs did not affect the results. More tests were performed with larger and smaller values of those variables, but they did not change the outcome. So, the best result is the following:
\begin{figure}[H]
	\centering
...
We can see in Figure \ref{result ekf mini with walls} that the position along the direction estimated by the odometry is incorrect, but changing the value in the process noise matrix did not improve the estimation. This is a correction that the AMCL node is intended to address.
\newpage
...
\end{figure}
\newpage
Having seen the results and that the changes to the process noise covariance did not affect the outcomes, it was decided to remove this data and evaluate the results using $v_{yaw}$ from only the IMU (data 001):\\
...
It overestimates the position, so we tried summing $v_{yaw}$ from both the IMU and the odometry provided by the Jobot (data 002):\\
...
Since this was the best outcome, we decided to change the process noise covariance.\\ All the results are:
...
Changing $\alpha 1$ and $\alpha 2$ fixes the rotation error, but we observe significant oscillation in the position correction if we increase them too much; this is evident in data 004. In the end, we decided to keep both parameters around 0.1. As a result, we obtained this picture of the odometry, which shows that the rotation was corrected. However, it also indicates that the positional movement has not been corrected, as expected, because the change in direction to the left occurs with respect to the map prior to the intersection of the hallways.
...
This allowed us to observe the corrections made for the heading. It is evident that the error increases as the robot moves, and we can see that we performed a good correction using the \texttt{robot\_localization} package.
...
\begin{figure}[H]
	...
	\captionof{figure}{Heading correction for the Mini with AMCL}
\end{figure}
\newpage
The table shows that without adding the correction for rotation, the translation error alone does not solve the problem. During testing (data 009), it was possible to see that the combination of these parameters allowed for a correction of the position on the left side of the path (the one with more map detail). This did not completely resolve the issues on the right side (with less detail), but it was able to make some corrections. The final solution (data 010) was the best, effectively addressing the position error during movement.
\begin{figure}[H]
	\centering
...
This shows that the small step generated a very high error in all three dimensions: $x$, $y$ and $yaw$, but AMCL managed to correct it perfectly.
...
One solution could be to rotate beforehand, allowing the AMCL to correct the error between the odometry frame and the map frame by adjusting the parameter that regulates the error caused by rotation. This can be implemented in openRMF by adding a vertex before the door to help correct the translation error, enabling the generation of a path to enter through the door. Another solution is to improve the odometry estimation of the Mini by using absolute reference markers visible to a camera, allowing the localization node to have periodic absolute corrections, similar to GPS. This enhancement can also facilitate corrections and movement outside the facility, allowing navigation between buildings to correct its absolute position.

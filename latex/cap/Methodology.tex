...
where:\\
- $H$ is the observation matrix, an identity matrix with not zero value on the colums of the measured variables.\\
...
\subsection{setting the ekf of the mini}

For the mini, that has and IMU and the odometry from the robot as data aviable from the sensors we choose to use only the $v_{yaw}$ of the gyroscope and the $v_x$ and $v_y$ from the odometry because the $yaw$ angle given by the orientation msg of the IMU have some random changing values:
and also the acceleration from the IMU was not be possible used because was adding a shift to the position.\\

More information on the sensor data used are the frequency of the topics:\\
\texttt{/odometry/wheels} : 15 Hz\\
\texttt{/imu/data} : 100 Hz\\
...
Was also tried to adding the $v_{yaw}$ of the odometry of the robot but it incremented the error.

The sensor avaible are:
IMU
\begin{itemize}
	\item orientation : cannot be used for an error from the sensor as visible in this picture
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.9\linewidth]{img/comparazione_heading_with_yawVel.png}
		\captionof{figure}{Heading result given by the sensors}
	\end{figure}
	\item gyroscopic : only real information about how the robot move, we take all the information about $v_{yaw}$
	\item acceleration : add a shifting position to the odometry can increment only the error
\end{itemize}
odometry
\begin{itemize}
	\item pose : as visible from the picture is not good so we update it using the localization node
	\item velocity : input given to the robot and the direct information about velocity, we will take $v_{x}$ and $v_{y}$
\end{itemize}

Using that input we tried changing the value of the process noise covariance $Q$ tring to obtain the best result for the different records, this test were performed using the record 001 where the start position is inside the office and the end is outside the door of the office ( a distance of about 4 m ) with a total length of about 187 m :
...
The plotted result are the following: 

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{img/plot_odom_withVyawOdom_01.png}
	\captionof{figure}{Results data 005}
\end{figure}

while all the test are desplied in the following image:
\begin{figure}[H]
	\centering
...
whit the following best result:
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{img/result_001_ekf.png}
	\captionof{figure}{Best result with 001 with the information of the map "acquired"}
\end{figure}

Where conducted test inserting also the data from the position and the velocity and couse a matrix parameter real big and small, this didnt change the result.

\subsection{setting the ekf of the jobot}
In both record the robot after a door in the record make a "jump" with rotation to the right visible in the odometry, this can be caused that in that position there is a holding position for the anti flame door. 
 from the firsts test it look like no much correction it need for the movment, it has only an hight error in the heading so the test where performed 
\begin{table}[h]
	\centering
	\begin{tabular}{|c|c|c|}
		\hline
...
Measure where performed with only $yaw$ 000, $V_{yaw}$ gyro 001, $V_{yaw}$ gyro and $V_{yaw}$ odom from the 002 and down.
The best result is with a coefficient of 320 with both the measure of velocity.

In this image is visible the data obtained in record 000 with only yaw and we see also changing the value of the coefficien matrix that the value doesnt change with also the Vyaw inserted\\

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{img/with_yaw.png}
	\captionof{figure}{with\_yaw}
\end{figure}

Then it was decided to remove it and see the result with the Vyaw of the imu\\

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{img/jobot_only_imu_Vyaw_bassa.png}
	\captionof{figure}{jobot\_only\_imu\_Vyaw\_bassa}
\end{figure}
the result are over exagerated. at the end we sum up the both of them to obtain \\

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{img/with_both_Vyaw.png}
	\captionof{figure}{with\_both\_Vyaw}
\end{figure}
that was promising and we change the value of the coessicient like in the table.
At the end we obtain this best result.\\


\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{img/final_best.png}
\captionof{figure}{Ottimal result obtained visualized in Rviz}
\label{Ottimal result obtained visualized in Rviz}
\end{figure}

The complete plot is:
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{img/jobot_ekf_full.png}
	\captionof{figure}{Some results plotted for the jobot setting parameter}
\end{figure}
\section{Parameter tuning for the AMCL}
Calibrating the Q now we have to correct to have the correct position also in function of the map, like described before. This compesnsate all the error given by not correct odometry estimation.
In this section we will edit the $\alpha$ parameters ans see the output. 
This parameters are:
...
The also other parameters is going to be tested. but this are first to look for changing  dinamics.

\subsection{setting the amcl of the mini}
this test were performed on record 007 and the start position is about 0.5m from the ending position. The choise to use this is because it was done in a challenging situation that show the difficult that may encounter a AMR. the problem is that for the first half of the path the robot encounters enough feature that can be used for the AMCL node to retrive the correct position respect the map (the doors of that part of the corridor are open and give sharp edge to the laser scanner), while in the second half the are not high detailed enviroment descriptor and the estimation of the position is going crazy (the door of the other half of the corridor are closed).


this is the odometry with alpha parameters set to 0:
\begin{figure}[H]
	\centering
...
the first row is the data from odometry given by the localization node and without correction of the AMCL node.
From the mini setting ekf parameters we saw that we need to compensate an error during rotation and also an error on position travelled. This can be compensate using the parameters: $\alpha 1$ and $\alpha 3$. so we decided t investigate more on the result changing that parameters.


changin the rotation parameter fix the rotation error, but we obtain a lot of oscillation in the psotion in data 004.
...
yhis are the plot of the heading:
...
\begin{figure}[H]
	...
	\captionof{figure}{a1\_amcl\_stain\_but\_notlenght}
\end{figure}
so the correction we eill apport for the heading are 0.001 for $\alpha 1$ e $\alpha 2$


changing only the $\alpha 3$ doest change the outcome 
while the combination fixed the error comming back from the rotation has not solved the problem at the right

At the end
the best solution is the last one that correct the position along the path:
...
plotting the data acquired we obtain:
\begin{figure}[H]
	\centering

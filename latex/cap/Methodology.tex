...
The UGV features a USB interface that allows connection to a PC or single-board computer for control purposes. In this work, a mini PC is used to control the UGV and also used to manage the entire navigation stack based on Nav2.
...
\end{figure}\\
A \textbf{2D laser scanner} is employed to provide environment perception and is used to compensate for odometry drift thanks the localization \ref{ref_map_to_odom} which provide a transformation link between the \texttt{map} and \texttt{odom} TF frames.
...
\newpage
For the Mini, we chose to use two distinct paths for the different types of configurations to be performed. For the parameters associated with the \texttt{robot\_localization} package, we followed a long path with many turns:\\
...
and this one for setting the parameters for the AMCL node because,on the right side of the door, along the corridor, the doors are closed and can reduce the robot's ability to accurately determine its position, making it crucial to fine-tune the AMCL parameters accordingly:
...
\end{figure}
\newpage
For the Jobot, the path is the following one and it has similarities with the second paths used for the Mini. After passing through the hallway, the doors were closed to assess how much this would affect the AMCL.\\
...
To reduce the testing time, we played the data at a rate five times higher using the flag \texttt{-r 5} for \texttt{robot\_localization} , while for the AMCL we used \texttt{-r 2}; this flag increases the output rate of each topic by n times. Tests were performed with rates from 1 to 5 for multiple recordings, and the output showed that the results of \texttt{robot\_localization} were similar. While the result for the AMCL was similar only with a rate of 2.\\
...
\end{figure}
\newpage
Having seen the results and tested some changes to the process noise covariance did not affect the outcomes, it was decided to remove this data and evaluate the results using $v_{yaw}$ from only the IMU (data 001):
\begin{figure}[H]
	\centering
...
It was present an hight oscillation movment when it should go in a straight line. This was solved by adjusting the parameters for the MPPI local planner, following the guide in the documentation.
...
\item To improve the safety the Collision Monitor\footnote{\href{https://github.com/ros-navigation/navigation2/tree/main/nav2\_collision\_monitor}{https://github.com/ros-navigation/navigation2/tree/main/nav2\_collision\_monitor}} was inserted. This node\ref{fig:CollisionMonitor} uses the information from the sensor used to detect obstacles to reduce the velocity sended to the robot.
If an object is inside the slowdown box it reduce its velocity, it send no velocity if an object is inside the stop box or it reduce the velocity if it estimate that a collision using the approach box is possible in the future\ref{fig:collision_poligons}.
\begin{figure}[H]
	\centering
